...
keys = [(RSA.generate(2048), None) for _ in range(num_keys)]
public_keys = [key.publickey().export_key() for key, _ in keys]
# 生成Merkle树并获取根节点
...
private_key, _ = keys[random_index]
...
print(f"Merkle Tree Generation Time: {merkle_tree_gen_time:.4f} seconds")
print(f"Signature Generation Time: {signature_gen_time:.4f} seconds")
print(f"Merkle Root Verification Time: {merkle_ver_time:.4f} seconds")
